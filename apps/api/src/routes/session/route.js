...
import { z } from "zod";
function extractMultiSessionCookieValue(cookieHeader, sessionToken) {
...
function hasMainSessionCookie(cookieHeader) {
    for (const cookie of cookieHeader.split(";")) {
        const eqIdx = cookie.indexOf("=");
        if (eqIdx === -1)
            continue;
...
export const sessionRoute = new Hono().post("/api/session/set-active", async (c) => {
    ...
    if (!hasMainSessionCookie(existingCookies)) {
        const signedValue = extractMultiSessionCookieValue(existingCookies, parsed.data.sessionToken);
        if (signedValue) {
            headers.set("cookie", `${existingCookies}; better-auth.session_token=${signedValue}`);
        }
    }
