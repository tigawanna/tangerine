...
const adminRoute = new Hono()
    ...
    .get("/admin/organizations", async (c) => {
    const { searchTerm, page, perPage, sortBy, sortOrder } = c.req.query();
    const pageNum = parseInt(page || "1") || 1;
    const perPageNum = parseInt(perPage || "30") || 30;
    const offset = calculateOffset(pageNum, perPageNum);
    try {
        let conditions = [];
        if (searchTerm) {
            conditions.push(or(ilike(organization.name, `%${searchTerm}%`), ilike(organization.slug, `%${searchTerm}%`)));
        }
        ...
        return c.json({ status: "error", message: error instanceof Error ? error.message : "Failed to fetch organizations" }, 500);
    }
})
    .get("/admin/organizations/:id/members", async (c) => {
    ...
    const offset = calculateOffset(pageNum, perPageNum);
    try {
        let conditions = [eq(member.organizationId, orgId)];
        if (searchTerm) {
            conditions.push(or(ilike(member.role, `%${searchTerm}%`), ilike(user.name, `%${searchTerm}%`), ilike(user.email, `%${searchTerm}%`)));
        }
        const [{ count: totalItems }] = await db
            .select({ count: count() })
            .from(member)
        ...
        const membersData = await db
            ...
            .limit(perPageNum)
            .offset(offset)
            .orderBy(buildOrderBy({ sortBy, sortOrder, columnMap: { role: member.role, createdAt: member.createdAt }, defaultColumn: member.createdAt }));
        ...
        return c.json({ status: "error", message: error instanceof Error ? error.message : "Failed to fetch members" }, 500);
    }
})
    ...
    try {
        ...
        return c.json({ status: "error", message: error instanceof Error ? error.message : "Failed to fetch users" }, 500);
    }
})
    .post("/admin/add-to-organization", async (c) => {
